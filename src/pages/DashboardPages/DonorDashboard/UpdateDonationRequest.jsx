...
const UpdateDonationRequest = () => {
  ...
  const onSubmit = (data) => {
    const donationRequest = {
      ...
      recipientDistrict:
        districts.find((d) => d.id === data.recipientDistrict)?.name ||
        donationRequest?.recipientDistrict,
      recipientUpazila:
        upazilas.find((u) => u.id === data.recipientUpazila)?.name ||
        donationRequest?.recipientUpazila,
      hospitalName: data.hospitalName,
      fullAddress: data.fullAddress,
      bloodGroup: data.bloodGroup,
      donationDate:
        data.donationDate.toISOString().split("T")[0] ||
        donationRequest.donationDate,
      donationTime:
        data.donationTime.toTimeString().slice(0, 5) ||
    ...
    //update the data
    axiosSecure
      .patch(`/update-donation-request/${id}`, donationRequest)
      .then((res) => {
        console.log(res.data);
        if (res.data.modifiedCount) {
          sweetMessage("Donation request updated successfully.");
        }
      })
    ...
    <div className="max-w-4xl w-full mx-auto p-6 bg-white shadow-md rounded">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 w-full">
        ...
        <div>
          ...
          <input
            defaultValue={donationRequest?.addressLine}
            className="input input-bordered w-full"
